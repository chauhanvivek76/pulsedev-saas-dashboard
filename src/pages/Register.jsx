...
export const Register = () => {
  ...
  const { logInfo, logSubmit, logClick } = useLogger('RegisterPage');
  ...
  useEffect(() => {
    logInfo('Register page rendered');
    if (isAuthenticated) {
      navigate('/dashboard');
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    logSubmit('Register Signup Form', { email: formData.email, name: formData.name });

    if (!validate()) {
      toastError('Please resolve the validation errors');
      return;
    }
    ...
    try {
      ...
      success('Account registered successfully! Welcome aboard.');
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 transition-colors duration-200 select-none">
      <Link 
        to="/" 
        onClick={() => logClick('Brand logo link (from Register)')}
        ...
        <CardContent className="p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1.5">
              Access the ultimate logging visualization stack
            </p>
          </div>
          ...
          <p className="mt-6 text-center text-xs text-slate-500 dark:text-slate-400">
            ...
            <Link
              to="/login"
              onClick={() => logClick('Switch to Login link')}
